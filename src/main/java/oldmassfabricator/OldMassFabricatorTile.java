...
public class OldMassFabricatorTile extends TileEntity implements IInventory, ISidedInventory, IEnergySink, IWrenchable, IEnergyReceiver {
    ...
    private int maxstoreRF = 100000000;
    private int needRFEnergy = 100000;
    private int storeRF = 0;
    private int maxReceiveRF = 100000;
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            ...
            // clientとの同期
            if (numUsing > 0) {
                PacketHandlerOMF.INSTANCE.sendToAll(new OMFSyncPKT((double) this.storeRF, this.scrap, this.xCoord, this.yCoord, this.zCoord));
    ...
    public void checkCreateRFMatter() {

        ItemStack stack = this.OMFItemStacks[1];


        if ((stack == null ? true : stack.stackSize < this.getInventoryStackLimit()) && this.storeRF >= this.needRFEnergy) {
            //if(this.storeRF >= 0)
            //{
            ...
            this.storeRF -= this.needRFEnergy;
    ...
    public int getRFProgress() {
        return ((this.storeRF / this.needRFEnergy) * 100);
    }
    ...
    public int addEnergy(int minReceive, int maxReceive, boolean simulate) {
        int amountReceived = Math.min(maxReceive, maxstoreRF - storeRF);
    ...
    public int receiveEnergy(ForgeDirection from, int maxReceive, boolean simulate) {
        // TODO 自動生成されたメソッド・スタブ
        return addEnergy(0, Math.min(maxReceive, this.maxReceiveRF), simulate);
    }
}
